...
| Component | Purpose |
|---|---|
| `AddExpenseForm` / `AddExpenseSheet` | Add-expense form, mobile sheet + desktop modal branches. Desktop (`add-expense/DesktopPanel.tsx`, design E4) is a 560px dialog: filing chips (group · date · category), What for / How much / Note wells, a stubbed "Split by item" row, and the token sentence. The split token slides `SplitLedger` in beside the form and widens the dialog to 1010px (below the sentence under a 1080px viewport); the footer states the outcome from `previewSplits` |
...
| `Token`, `PersonToken` | The app's one selectable-pill shape (`components/PersonToken.tsx`). `Token` takes optional leading content (avatar/emoji/icon) + label; `PersonToken` wraps it for a group member. Warm-white raised, sun gradient when selected, dips on press. **Renders flat when given no `onClick`** — a pill you can only read is information, not a control. Used by the add-expense date sheet's quick picks, the activity page, the expense sheet's payer picker, and read-only split chips |
| `Input` | Recessed-well text input (`components/Input.tsx`) — owns the well, the borderless field, `prefix`/`suffix` affixes, and its own focus state (sun rim, replacing the native outline). Sizes `hero` (amount) / `title` (description) / `field` + `fieldLg` (desktop add dialog's paired description + amount) / `md` / `cellLg` / `cell` (split cells). Inputs whose well holds more than a value — `MemberCombobox`'s chips, `HandleInput`'s validation pip — compose `well()` from `design/tokens.ts` directly instead |
| `Segmented` | Recessed track + raised warm-white insert. Backs the add-expense split-mode tabs (mobile split sheet, desktop ledger). Deliberately **not** sun-filled — the sun accent is reserved for the one primary action per view |
